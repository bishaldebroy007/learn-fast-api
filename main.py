...
class Patient(BaseModel):
    ...
    @computed_field
    @property
    def verdict(self) -> str:  ## Decision making based on BMI value, to determine the health status of the patient.
...
def creat_patient(patient: Patient): # here "patient" is a pydancic object
    ...
    # Now if things are alright, add those to database: add patient to pydantic object(patient)
        ...
        data[patient.id] = patient.model_dump(exclude=["id"]) # Convert the object to python disctionary using, model_dump()
